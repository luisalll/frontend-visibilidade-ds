...
export default function TemplateEditPage({ params }) {
  ...
  const handleDownload = async () => {
    const response = await fetch(`${process.env.API_URL}/${template.imagePath}`);
        ...
        <div className="flex flex-1 gap-8">
          <div className="w-1/2 flex flex-col gap-4">
            <div className="flex items-center gap-2 mb-2">
              <Button
                className="bg-white text-gray-900 hover:bg-gray-100 shadow-md cursor-pointer p-2 dark:text-white"
                size="xs"
                variant="outline"
              >
                Adicionar Texto
            ...
            {template && (
              <div className="bg-gray-200 dark:bg-gray-700 rounded-lg aspect-square flex items-center justify-center relative group">
                <img src={`${process.env.API_URL}/${template.imagePath}`} alt={template.name} className="w-full h-full object-cover" />
              </div>
            )}

            <div className="mt-2">
              <Button
                className="w-full bg-gray-100 hover:bg-gray-200 text-gray-900 border border-gray-300 cursor-pointer"
                size="sm"
          ...
          <div className="w-1/2 space-y-6">
            ...
            <div>
              <label htmlFor="description" className="block text-sm font-medium mb-2">
                Gere um texto criativo para o template
              </label>
              <Input
                id="description"
                placeholder="Descreva brevemente seu template"
                value={description}
            ...
            <div className="pt-4 flex gap-4 w-full">
              <Button
                className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-900 border border-gray-300 cursor-pointer"
